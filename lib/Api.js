...
class Api extends Homey.SimpleClass {
    ...
    init() {
        ...
        this._refreshToken = Homey.ManagerSettings.get('refreshToken');

        Homey.ManagerApi.realtime('com.ring.status', {
            state: 'api_init'
        });
    ...
    _https_auth(callback) {
        ...
        console.log('_https_auth : Post Data : ', postdata);
        ...
        console.log('_https_auth : Headers : ', url.headers);
        ...
        let request = https.request(url, (response) => {
            ...
            response.on('end', () => {
                ...
                if (response.statusCode >= 400) {
                    ...
                    try {
                        result = JSON.parse(data);
                        console.log('_https_auth : Result : ', result);
                        this._refreshToken = result.refresh_token;
                        Homey.ManagerSettings.set('refreshToken', this._refreshToken);
                    } catch (e) {
                        error = e;
                    }
                }
